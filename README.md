...
- `content/locales/<locale>/index.html` homepage source files such as `content/locales/ja/index.html` and `content/locales/zh/index.html` are generated from `index.html` before builds and ignored by Git. Vite emits them at public routes such as `/ja/` and `/zh/`.
- `support.html` is the static support page.
- `legal/` contains clean English legal pages, legacy English `.html` compatibility pages, and translated legal `.html` pages. Vite emits them at the existing public legal URLs.
- `content/pairs/*/index.html` directories are clean-URL SEO pages for English minimal-pair sound contrasts and SEO hubs.
- `content/locales/*/*/index.html` directories are localized SEO pages and localized SEO hubs.
...
- `vite.config.js` groups HTML entry points by page type, reads source HTML from `content/` and `legal/`, and preserves the existing public output paths.
...
For a clean URL SEO page such as `https://getsoundwise.co/example-page/`:
...
Localized homepage routes such as `/ja/` and `/zh/` are generated by `npm run generate:localized-homepages` into `content/locales/<locale>/index.html`; do not hand-maintain those route `index.html` files.

For a translated legal page, keep the existing public naming pattern but store the source in `legal/`:

- `legal/privacy/privacy-[locale].html`
- `legal/terms/terms-[locale].html`

Then add the locale code to `legalLocales` in `vite.config.js` and update the legal page language switchers. The build maps those source files back to root public `.html` URLs such as `/privacy-ja.html`.

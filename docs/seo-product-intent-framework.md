...
GA4 supports this natively: `exercise_id` (the contrast) and `learner_language` (`document.documentElement.lang`) are already sent as event params on every funnel event (`src/seo-page.js` → `buildExerciseParams`), and `page_path` is sent on `app_store_click`. No new instrumentation is required to build all three rollups — this is a reporting/Exploration-report construction task, not a code change. `learner_language` is active learner/UI context, not proof of native language or L1; `locale` remains the canonical route locale.
